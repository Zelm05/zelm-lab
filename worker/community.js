...
import { authenticate, verifySession, json, checkRateLimit, getClientIP } from './auth.js';
import { getSetting } from './settings.js';
import { logModeration } from './moderation.js';

/**
 * 统一会话校验（P0-2）。
 * 返回 { user } / { res }（res 是要直接返回的 401 Response，已含顶号提示）。
 *
 * 为什么写接口一律走 verifySession：登出 / 被顶号只删 sessions 行，JWT 并未吊销，
 * 用 authenticate() 的话旧令牌能在 7 天内继续发留言、删回复、读自己的反馈。
 */
async function requireSession(request, env) {
  const user = await verifySession(request, env);
  if (!user) return { res: json({ error: '请先登录' }, 401) };
  if (user.kicked) return { res: json({ kicked: true, message: '账号已在其他设备登录' }, 401) };
  return { user };
}
...
export async function listMessages(request, env) {
  ...
  const rows = await env.DB
    .prepare('SELECT m.id, m.user_id, m.username, m.content, m.likes, m.created_at, u.username AS current_name FROM messages m LEFT JOIN users u ON u.id = m.user_id WHERE m.deleted_at IS NULL ORDER BY m.id DESC LIMIT ? OFFSET ?')
    .bind(limit, offset)
    .all();
...
export async function postMessage(request, env) {
  const user = await verifySession(request, env);
  // 免登录模式下游客（无 Cookie）也要放行；但**被顶号**必须拦下，
  // 否则旧令牌还能冒名发留言
  if (user && user.kicked) return json({ kicked: true, message: '账号已在其他设备登录' }, 401);
  const requireLogin = await messageLoginRequired(env);
...
export async function deleteReply(request, env, messageId, replyId) {
  ...
  // P2-6：两条删除合并为一次 db.batch
  // P2-22：追加一条审计日志（回复表无 deleted_at，仍为物理删除）
  await env.DB.batch([
    env.DB.prepare('DELETE FROM message_replies WHERE parent_reply_id = ?').bind(replyId),
    env.DB.prepare('DELETE FROM message_replies WHERE id = ?').bind(replyId),
    env.DB.prepare('INSERT INTO moderation_log (actor_id, actor_username, action, target_type, target_id, created_at) VALUES (?, ?, ?, ?, ?, ?)')
      .bind(Number(user.sub), String(user.username), 'delete_reply', 'reply', replyId, Date.now()),
  ]);
...
export async function deleteMessage(request, env, id) {
  ...
  // P2-22：软删（置 deleted_at），保留记录便于审计；子表（点赞/回复）物理删除
  //        （父已不可见，子无独立意义）。四条写操作合并为一次 db.batch，避免中间态。
  const now = Date.now();
  await env.DB.batch([
    env.DB.prepare('UPDATE messages SET deleted_at = ? WHERE id = ?').bind(now, id),
    env.DB.prepare('DELETE FROM message_likes WHERE message_id = ?').bind(id),
    env.DB.prepare('DELETE FROM message_replies WHERE message_id = ?').bind(id),
    env.DB.prepare('INSERT INTO moderation_log (actor_id, actor_username, action, target_type, target_id, created_at) VALUES (?, ?, ?, ?, ?, ?)')
      .bind(Number(user.sub), String(user.username), 'delete_message', 'message', id, now),
  ]);
...
export async function myFeedbacks(request, env) {
  ...
  const rows = await env.DB
    .prepare('SELECT id, kind, content, reply, replied_at, created_at FROM feedbacks WHERE user_id = ? AND deleted_at IS NULL ORDER BY id DESC')
    .bind(user.sub)
    .all();
...
export async function adminFeedbacks(request, env) {
  ...
  const countConds = [];
  const params = [];
  // P2-22：软删记录一律不出现在管理台列表 / 统计中
  conds.push('f.deleted_at IS NULL');
  countConds.push('deleted_at IS NULL');
  ...
  const stats = await env.DB
    .prepare("SELECT COUNT(*) AS total, SUM(CASE WHEN reply IS NULL THEN 1 ELSE 0 END) AS pending FROM feedbacks WHERE deleted_at IS NULL")
    .first();
...
export async function replyFeedback(request, env, id) {
  ...
  const item = await env.DB
    .prepare('SELECT id FROM feedbacks WHERE id = ? AND deleted_at IS NULL')
    .bind(id)
    .first();
  ...
  // P2-22：审计日志
  await logModeration(env, user.sub, user.username, 'reply_feedback', 'feedback', id);
...
export async function deleteFeedback(request, env, id) {
  ...
  // P2-22：软删（置 deleted_at）+ 审计日志，一次 batch 提交
  await env.DB.batch([
    env.DB.prepare('UPDATE feedbacks SET deleted_at = ? WHERE id = ?').bind(Date.now(), id),
    env.DB.prepare('INSERT INTO moderation_log (actor_id, actor_username, action, target_type, target_id, created_at) VALUES (?, ?, ?, ?, ?, ?)')
      .bind(Number(user.sub), String(user.username), 'delete_feedback', 'feedback', id, Date.now()),
  ]);
