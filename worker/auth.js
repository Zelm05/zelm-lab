...
//   4) 鉴权中间件         —— authenticate() / verifySession()
//                            ⚠️ 需要登录的接口一律用 verifySession()，不要用 authenticate()
// ===================================================================
...
// P2-5：导出供 api.js 的登录重算逻辑引用（避免重复硬编码）。
...
//
// ⚠️ 只验 JWT 签名，**不查 sessions 表**。登出 / 被顶号的实现是删除 sessions 行
//    （见 api.js 的 logout / adminKickUser），JWT 本身并未被吊销，在 7 天有效期内
//    依然能通过这里。因此：
//      · 需要登录的接口  → 用下面的 verifySession()
//      · 只想读 payload 且允许"已登出但仍有效的令牌"（只有 logout() 一个场景）
//                       → 才用 authenticate()
...
// 会话校验（单端登录）：在 authenticate() 之上再查一次 sessions 表
//
// 返回三种结果，调用方必须都处理：
//   · 签名无效 / 无 Cookie  → null                 （等同未登录）
//   · 会话已被登出或顶号    → { kicked: true }     （令牌仍有效，但服务端已不承认）
//   · 正常                  → payload，并顺带刷新 last_seen 心跳
//
// 副作用：每次调用会写一次 `UPDATE sessions SET last_seen`。
// 只读且允许游客的接口（如 GET /api/messages）可继续用 authenticate() 省掉这次写。
