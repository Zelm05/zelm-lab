...
// 读取全部站点设置（归一化后的对象）
//
// P2-17：原实现对 DEFAULTS 的每个 key 各发一条 SELECT（7 个 key = 7 次 D1 往返），
//   而 getSiteSettings() 在**每个 HTML 请求**上都会被调用一次（Worker 里 run_worker_first，
//   要把配置写进 Cookie 供前端同步读）。这里合并成一条 `WHERE key IN (?,?,…)`。
// 语义与原实现逐条对齐：DB 里没这一行 / 值为空串 → 回退 DEFAULTS[key]。
export async function getSiteSettings(env) {
  const keys = Object.keys(DEFAULTS);
  const out = {};
  // 先用默认值铺底：表不存在、缺行、或值为空串时都落在这里
  ...
  try {
    ...
    /* 表不存在时静默回退默认值（与 getSetting 的容错一致） */
  }
  return out;
}
...
export async function updateSettings(request, env) {
  // P0-2：站点设置是站长专属写接口，必须查 sessions 表
  ...
  try {
    // P2-6：多个键值写入合并为一次 db.batch，减少 D1 往返
    await env.DB.batch(Object.keys(patch).map((key) =>
      env.DB.prepare('INSERT INTO site_settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value')
