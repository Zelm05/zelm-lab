...
/* 站点头像统一走内容 store（后台「关于我」可换）；未上传时回落到内置图。
   ⚠️ 这里**不**调 ensure('about')：AuthPanel 常驻在 App.vue，顶层拉取会让
   **每次页面加载**都多一次 /api/content/about 请求，而绝大多数访客不会打开登录框。
   改成打开时才取（见下面 visible 的 watcher）。 */
...
<template>
  ...
  <div class="auth-modal" :hidden="!a.visible" role="dialog" aria-modal="true">
    ...
    <div class="auth-card">
      ...
      <div class="auth-head">
        <img :src="content.avatarUrl" :alt="tc('avatarAlt')" width="256" height="256" />
