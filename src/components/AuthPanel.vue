    ...
    <div class="auth-card">
      ...
      <div id="apPanelRegister" class="auth-panel" :hidden="a.tab !== 'register'">
        <form id="apRegForm" novalidate @submit.prevent="a.doRegister()">
          ...
          <!-- 头像选择（注册时选一个预设头像，存到 users.avatar） -->
          <div class="auth-field">
            <label>{{ a.T.avatarLabel }}</label>
            <div id="apAvatarPicker" class="avatar-picker" role="radiogroup" :aria-label="a.T.avatarLabel">
              <button
...
.avatar-option.active {
  border-color: var(--accent, #4ff0d0);
  background: color-mix(in srgb, var(--accent, #4ff0d0) 18%, transparent);
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--accent, #4ff0d0) 22%, transparent);
}
