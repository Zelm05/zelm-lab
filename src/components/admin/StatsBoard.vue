...
async function ensureInit() {
  ...
  if (!pieEl.value || !barEl.value) {
    if (a.statsReady) { failed.value = '图表容器未渲染（pieEl/barEl 为空）'; loading.value = false; }
    return;
  }
  if (!pieInst) {
    try {
    ...
    } catch (err) { failed.value = 'echarts.init 失败：' + (err && err.message ? err.message : err); loading.value = false; return; }
  ...
  try { render(); } catch (err) { failed.value = '图表渲染失败：' + (err && err.message ? err.message : err); }
  loading.value = false;
}
