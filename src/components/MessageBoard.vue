      ...
      <template v-else>
        <div v-for="m in shown" :key="m.id" class="msg-item">
          ...
          <div class="msg-actions">
            <el-button size="small" class="msg-like" :class="{ liked: m.liked }" :data-like="m.id" :disabled="busy['like:' + m.id]" @click="toggleLike(m)">{{ (m.liked ? '❤️ ' : '🤍 ') + m.likes }}</el-button>
            <el-button
