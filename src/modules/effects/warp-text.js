...
(function (global) {
  ...
  function createWarp(container, options, getTextureCanvas) {
    ...
    /* P2-10：GL 资源创建抽成函数。
       上下文丢失时这些对象（texture / geometry / program / mesh）在 GPU 侧全部失效，
       恢复后必须用**同一套参数**重建一遍 —— 所以创建逻辑只写一份、两处调用。 */
    ...
    /* P2-10：上下文恢复。
       此前只处理了 lost —— contextLost 一旦置 true 就再没人能置回来，
       renderOnce / rasterize / resize / loop 全部早退，特效**永久黑屏**。
       真实触发场景：GPU 驱动重置（TDR）、标签页长时间后台被浏览器回收上下文、
       同页 WebGL 上下文数量超上限被逐出。
       恢复后：① 重建失效的 GL 对象；② 复位渲染状态；③ 重新光栅化并重启循环。
       ⚠ 已知残留：ogl 是压缩打包的库、没有对外暴露状态重置 API，它内部的 GL
       状态缓存无法清空，所以真实（非合成）丢上下文后头几帧的混合/深度状态可能是
       旧值。彻底解决要等 ogl 提供 reset，不属本项范围。 */
    function onContextRestored() {
      ...
      try {
        ...
        console.warn('Warp: 上下文恢复后重建 WebGL 资源失败，特效保持关闭', e);
        contextLost = true;
        return;
      }
      resize();     // 内部重设 dpr / 视口 / uResolution，并 rasterize() → texture.needsUpdate
    ...
    return {
      ...
      rasterize: rasterize,
      /* P2-10：验证用 —— 手动派发 webglcontextlost / webglcontextrestored 后读它，
         期望 lost 后为 true、restored 后回 false。 */
      getContextLost: function () { return contextLost; }
    };
  }
...
/** 原 window.WarpText —— 改为模块导出 */
export const WarpText = __expose.WarpText;
/** 原 window.WarpImage —— 改为模块导出 */
export const WarpImage = __expose.WarpImage;
