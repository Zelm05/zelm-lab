/* ==========================================================================
 * 由 public/legacy/lib/particle-text.js 经一次性迁移脚本（%TEMP%/zelm_vueify.cjs）
 * 转换而来；旧源 public/legacy/lib/particle-text.js 已删除，该脚本现仅为历史产物、
 * 无任何构建流程调用，本文件改由手写维护，可直接修改。
 * 纯 2D canvas 实现（不依赖 WebGL）。
 * ========================================================================== */
...
(function (global) {
  ...
  function ParticleText(container, options) {
    ...
    var gathering = false;
    var gatherStart = 0;
    /* P2-9：destroy 后必须彻底停摆。
     *   实测泄漏：ResizeObserver.disconnect() **不能**撤销已经排进微任务队列的那次回调，
     *   于是 destroy 之后落地的那次回调仍会走 sampleText → doSample → ensureRenderLoop，
     *   把 rAF 循环重新拉起来 —— 而且此时卸载器已经执行过，没有任何人再能取消它。
     *   GateView 切语言时会「先销毁旧实例再造新实例」，旧实例被复活后就是两条循环
     *   画同一张 canvas，且永久累积。 */
    var disposed = false;
    ...
    function render(now) {
      if (disposed) return;                       // P2-9：停摆后不再绘制、也不再自排下一帧
    ...
    function ensureRenderLoop() {
      if (disposed) return;                       // P2-9：这是循环的唯一入口，堵这里即可
      if (animationFrame === null) animationFrame = window.requestAnimationFrame(render);
    }

    function sampleText() {
      if (disposed) return;                       // P2-9：迟到的 resize / 字体回调不再重新采样
    ...
    function doSample(offCtx, offscreen, content, font, resolvedSize, _currentBuild) {
      if (disposed) return;                       // P2-9：异步落地后也要再确认一次
      var metrics = offCtx.measureText(content);
    ...
    function queueSample() {
      if (disposed) return;                       // P2-9：ResizeObserver 的迟到回调挡在这里
    ...
    return {
      /* P2-9：释放清单 —— 本文件是**纯 2D canvas**（`canvas.getContext('2d')`，
         全文件无 WebGL、无 ogl），所以不存在 WebGL 上下文丢失/恢复这回事，
         geometry / texture / program / renderer 四项在这里也不存在。
         本文件真正持有的资源与释放方式如下：
           · ResizeObserver        → disconnect()（下面第 1 条）
           · matchMedia 监听器      → removeEventListener
           · canvas 事件 × 4        → removeEventListener
           · rAF × 2（主循环/重采样）→ cancelAnimationFrame
           · 未落地的字体 Promise   → buildId 自增使其全部失效
           · particles（≤5200 个对象）→ 清空数组，立刻可回收
         故意**不**做的事：不移除 canvas 元素（模板里已有一个，实例销毁后
         重建时直接复用；移掉反而要重建 DOM）。 */
      destroy: function () {
        if (disposed) return;                    // P2-9：幂等
        disposed = true;                         // 先立标记，后面任何迟到回调都会短路
        buildId += 1;                            // 让所有未落地的异步结果失效
        ...
        /* 注意：这里刻意不给 animationFrame / resizeFrame 置 null。
           置 null 会让 ensureRenderLoop() 的 `animationFrame === null` 重新成立 ——
           那是给「复活」留门。保留已取消的旧 id 是第二道闸。 */
...
/** 原 window.ParticleText —— 改为模块导出 */
export const ParticleText = __expose.ParticleText;
