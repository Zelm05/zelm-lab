...
(function () {
  ...
  var css = [
    ...
    '.auth-card{position:relative;z-index:1;width:min(400px,92vw);background:rgba(13,24,19,.88);border:1px solid color-mix(in srgb, var(--accent) 28%, transparent);border-radius:22px;padding:26px 26px 22px;color:#e9edf6;box-shadow:0 24px 80px rgba(0,0,0,.55),0 0 44px color-mix(in srgb, var(--accent) 12%, transparent);backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);animation:authPop .32s cubic-bezier(.34,1.56,.64,1)}',
    ...
    '.auth-adminlink{display:block;margin:14px auto 0;border:none;background:rgba(255,255,255,.05);border:1px solid color-mix(in srgb, var(--accent) 25%, transparent);color:var(--accent);font-size:.76rem;font-family:inherit;padding:6px 16px;border-radius:999px;cursor:pointer;transition:all .2s}',
    '.auth-adminlink:hover{background:color-mix(in srgb, var(--accent) 12%, transparent);box-shadow:0 0 12px color-mix(in srgb, var(--accent) 25%, transparent)}',
  ...
  var T = {
    ...
    loginNameHint:{ zh: '用户名就是你的显示名，改名后请用新名字登录', en: 'Your username is your display name — use your new name to log in after renaming' },
    ...
    uHint:        { zh: '2-32 位，可包含汉字、字母、数字和下划线，不可与已有账号重复', en: '2-32 chars, Chinese / letters / digits / _, must be unique' },
